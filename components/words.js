...
function word_list(name, words, table, next_i) {
    ...
    var self = {
        ...
        max: 0,
        next_i: next_i || 0,
        how_many_more: function() {
            return self.length - self.next_i;
        },
        any_more: function() {
            return self.how_many_more() > 0;
        },
        ditLength: function(word) {
            return table.ditLength(word);
        },
        startAt: function(i) {
            self.next_i = i;
        },
...
function word_list_by_name(name, table, next_i) {
    switch (name) {
        case 'comb':
            return word_list_comb(table, next_i);
        case 'voa':
            return word_list_voa(table, next_i);
        case 'callsigns':
            return word_list_callsigns(table, next_i);
        default:
